...
{
    public class UpdatePublishingHouseCommandValidator
      : AbstractValidator<UpdatePublishingHouseCommand>
    {
        public UpdatePublishingHouseCommandValidator(IStringLocalizer localizer)
        {
            RuleFor(x => x.Id)
                .NotEmpty()
            ...
            // التحقق من الصورة فقط إذا تم توفيرها (اختياري)
            When(x => x.Image != null, () =>
            {
                RuleFor(x => x.Image!)
