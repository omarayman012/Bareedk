...
public class SalesAndPurchasePolicyController(IMediator mediator) : ControllerBase
{
    [HttpGet]
    public async Task<IActionResult> Get(CancellationToken cancellationToken)
    {
        var result = await mediator.Send(new GetSystemManagementQuery<SalesAndPurchasePolicy>(), cancellationToken);
        return StatusCode(result.StatusCode, result);
    }

    [HttpPut]
    public async Task<IActionResult> Update(
        [FromBody] UpdateSystemManagementCommand<SalesAndPurchasePolicy> command,
